...
// use App\Models\LevelModel; use App\Models\BarangModel; use App\Models\KategoriModel; use Illuminate\Http\Request;
// use Illuminate\Support\Facades\Hash;
// use Illuminate\Support\Facades\Validator; use PhpOffice\PhpSpreadsheet\IOFactory; use Yajra\DataTables\Facades\DataTables;
// class BarangController extends Controller
// {
...
// $kategori = KategoriModel::select('kategori_id', 'kategori_nama')->get(); return view('barang.index', [
// 'activeMenu' => $activeMenu, 'breadcrumb' => $breadcrumb, 'kategori' => $kategori
// ]);
// }
...
// $kategori_id = $request->input('filter_kategori'); if(!empty($kategori_id)){
...
// $btn .= '<button onclick="modalAction(\''.url('/barang/' . $barang-
// >barang_id . '/delete_ajax').'\')" class="btn btn-danger btn-sm">Hapus</button> '; return $btn;
// })
...
// public function create_ajax()
// {
// $kategori = KategoriModel::select('kategori_id', 'kategori_nama')->get(); return view('barang.create_ajax')->with('kategori', $kategori);
// }
// public function store_ajax(Request $request)
// {
// if($request->ajax() || $request->wantsJson()){
// $rules = [
// 'kategori_id' => ['required', 'integer', 'exists:m_kategori,kategori_id'], 'barang_kode' => ['required', 'min:3', 'max:20',
// 'unique:m_barang,barang_kode'],
// 'barang_nama' => ['required', 'string', 'max:100'], 'harga_beli' => ['required', 'numeric'], 'harga_jual' => ['required', 'numeric'],
// ];

// $validator = Validator::make($request->all(), $rules); if($validator->fails()){
// return response()->json([ 'status' => false,
// 'message' => 'Validasi Gagal', 'msgField' => $validator->errors()
// ]);
// }

// BarangModel::create($request->all()); return response()->json([
// 'status' => true,
...
//     $check = BarangModel::find($id); if ($check) {
...
//     {
//     if($request->ajax() || $request->wantsJson()){
//     $barang = BarangModel::find($id); if($barang){ // jika sudah ditemuikan
//     $barang->delete(); // barang di hapus return response()->json([
//     'status' => true,
...
// $insert = [];
// if(count($data) > 1){ // jika data lebih dari 1 baris foreach ($data as $baris => $value) {
// if($baris > 1){ // baris ke 1 adalah header, maka lewati
// $insert[] = [
// 'kategori_id' => $value['A'], 'barang_kode' => $value['B'], 'barang_nama' => $value['C'], 'harga_beli' => $value['D'], 'harga_jual' => $value['E'], 'created_at' => now(),
// ];
// }
// }

// if(count($insert) > 0){
// // insert data ke database, jika data sudah ada, maka diabaikan BarangModel::insertOrIgnore($insert);
// }
